...
<body>
    ...
    <script>
        ...
        let player = {
            x: canvas.width / 2,
            y: canvas.height / 2,
            radius: 63, // Обновляем радиус под размер RPG текстуры (126 высота) / 2
            health: 100,
            maxHealth: 100,
            speed: 5,
            angle: 0,
    ...
    levelsModal.style.display = 'flex';
        ...
        function selectWeapon(weaponType) {
            ...
            // Обновляем радиус игрока в зависимости от оружия
            if (weaponType === 'rpg') {
                player.radius = isMobile ? 50 : 63; // Больший радиус для RPG
            } else if (weaponType === 'rifle') {
                player.radius = isMobile ? 50 : 63; // Больший радиус для автомата
            } else if (weaponType === 'shotgun') {
                player.radius = isMobile ? 50 : 63; // Больший радиус для дробовика
            } else {
                player.radius = isMobile ? 40 : 50; // Обычный радиус для других оружий
            }
        ...
        function init() {
            ...
            if (isMobile) {
                player.radius = 50; // Обновляем радиус для мобильных устройств под размер RPG текстуры
                player.speed = 4;
                ...
                player.radius = 63; // Обновляем радиус для ПК под размер RPG текстуры
                player.speed = 5;
            }
        ...
        function spawnEnemy(type = null) {
            ...
            const enemy = {
                x: x,
                y: y,
                radius: enemyType.radius, // Default radius, will be updated for 'chase' type
            ...
            // Если враг использует текстуру, установить радиус на основе размера текстуры
            if (enemy.type === 'chase') {
                // Используем среднее значение ширины и высоты текстуры, деленное на 2
                enemy.radius = (ENEMY_TEXTURE_WIDTH + ENEMY_TEXTURE_HEIGHT) / 4;
            } else if (enemy.enemyClass === 'hunter-enemy') {
                // Устанавливаем радиус для охотника на основе размера текстуры
                enemy.radius = (HUNTER_TEXTURE_WIDTH + HUNTER_TEXTURE_HEIGHT) / 4;
            } else if (enemy.enemyClass === 'tank-enemy') {
                // Устанавливаем радиус для танка на основе размера текстуры
                enemy.radius = (TANK_TEXTURE_WIDTH + TANK_TEXTURE_HEIGHT) / 4;
            }
        ...
        function updatePlayer(dt = 1) {
            ...
            // --- РИСУЕМ ИГРОКА ТЕКСТУРОЙ ИЛИ ЗАПАСНЫМ КРУГОМ ---
            ctx.save();
            ctx.translate(player.x, player.y);
            
            // Поворачиваем текстуру в направлении курсора
            // Добавляем смещение -Math.PI/2 чтобы текстура смотрела вверх по умолчанию
            ctx.rotate(player.angle - Math.PI/2);
            
            // Проверяем, какое оружие выбрано и рисуем соответствующую текстуру
            if (currentWeapon.name === 'RPG' && playerRpgImg.complete && playerRpgImg.naturalWidth > 0) {
                // Рисуем текстуру игрока с RPG
                // Поворачиваем на 180 градусов (Math.PI) для правильной ориентации
                ctx.rotate(Math.PI);
                ctx.drawImage(playerRpgImg, 
                              -PLAYER_RPG_WIDTH / 2, -PLAYER_RPG_HEIGHT / 2, 
                              PLAYER_RPG_WIDTH, PLAYER_RPG_HEIGHT);
            } else if (currentWeapon.name === 'Rifle' && playerRifleImg.complete && playerRifleImg.naturalWidth > 0) {
                // Рисуем текстуру игрока с автоматом
                // Поворачиваем на 180 градусов (Math.PI) для правильной ориентации
                ctx.rotate(Math.PI);
                ctx.drawImage(playerRifleImg, 
                              -PLAYER_RIFLE_WIDTH / 2, -PLAYER_RIFLE_HEIGHT / 2, 
                              PLAYER_RIFLE_WIDTH, PLAYER_RIFLE_HEIGHT);
            } else if (currentWeapon.name === 'Shotgun' && playerShotgunImg.complete && playerShotgunImg.naturalWidth > 0) {
                // Рисуем текстуру игрока с дробовиком
                // Поворачиваем на 180 градусов (Math.PI) для правильной ориентации
                ctx.rotate(Math.PI);
                ctx.drawImage(playerShotgunImg, 
                              -PLAYER_SHOTGUN_WIDTH / 2, -PLAYER_SHOTGUN_HEIGHT / 2, 
                              PLAYER_SHOTGUN_WIDTH, PLAYER_SHOTGUN_HEIGHT);
            } else if (playerImg.complete && playerImg.naturalWidth > 0) {
                // Рисуем обычную текстуру игрока
                ctx.drawImage(playerImg, 
                              -PLAYER_TEXTURE_SIZE / 2, -PLAYER_TEXTURE_SIZE / 2, 
                              PLAYER_TEXTURE_SIZE, PLAYER_TEXTURE_SIZE);
            } else {
                // Запасной круг если текстуры не загружены
                ...
                // Добавляем стрелку направления для отладки
        ...
        function updateEnemies(timestamp, dt = 1) {
            ...
            enemies.forEach(function(enemy, index) {
                    ...
                    if (enemy.type === 'melee') {
                        ...
                        for (let i = -1; i <= 1; i++) {
                            ...
                            enemyBullets.push({
                                ...
                                speed: 5,
                                radius: ENEMY_BULLET_SIZE / 2, // Радиус коллизии под размер текстуры
                ...
                if (enemy.health > 0) {
                    // --- РИСУЕМ ВРАГА ТЕКСТУРОЙ ИЛИ ЗАПАСНЫМ КРУГОМ ---
                    ctx.save();
                    ctx.translate(enemy.x, enemy.y);
                    // Поворачиваем контекст в сторону игрока
                    const textureRotationOffset = -Math.PI / 2; // Пример: предполагаем, что текстура смотрит вверх по умолчанию
                    ctx.rotate(angle + textureRotationOffset);

                    // Проверяем, нужно ли рисовать текстуру (для 'chase' врагов, если текстура загружена)
                    const useTexture = enemy.type === 'chase' && enemyImg.complete && enemyImg.naturalWidth > 0;
                    
                    // Проверяем, нужно ли рисовать текстуру охотника
                    const useHunterTexture = enemy.enemyClass === 'hunter-enemy' && hunterImg.complete && hunterImg.naturalWidth > 0;
                    
                    // Проверяем, нужно ли рисовать текстуру танка
                    const useTankTexture = enemy.enemyClass === 'tank-enemy' && tankImg.complete && tankImg.naturalWidth > 0;

                    if (useTexture) {
                        // Рисуем текстуру, отцентрированную в точке вращения
                        ctx.drawImage(enemyImg, 
                                      -ENEMY_TEXTURE_WIDTH / 2, -ENEMY_TEXTURE_HEIGHT / 2, 
                                      ENEMY_TEXTURE_WIDTH, ENEMY_TEXTURE_HEIGHT);
                    } else if (useHunterTexture) {
                        // Рисуем текстуру охотника
                        ctx.drawImage(hunterImg, 
                                      -HUNTER_TEXTURE_WIDTH / 2, -HUNTER_TEXTURE_HEIGHT / 2, 
                                      HUNTER_TEXTURE_WIDTH, HUNTER_TEXTURE_HEIGHT);
                    } else if (useTankTexture) {
                        // Рисуем текстуру танка
                        ctx.drawImage(tankImg, 
                                      -TANK_TEXTURE_WIDTH / 2, -TANK_TEXTURE_HEIGHT / 2, 
                                      TANK_TEXTURE_WIDTH, TANK_TEXTURE_HEIGHT);
                    } else {
                        // Рисуем запасной круг для других типов врагов или если текстура не загружена
                        ctx.fillStyle = enemy.color;
                        ctx.beginPath();
                        ctx.arc(0, 0, enemy.radius, 0, Math.PI * 2); // Рисуем круг в (0, 0) относительно перенесенного контекста
                        ctx.fill();
                    ...
                    // Добавляем индикатор направления для отладки (необязательно)
                    ctx.strokeStyle = 'yellow';
                    ctx.lineWidth = 1;
                    ctx.beginPath();
                    ctx.moveTo(0, 0);
                    ctx.lineTo(enemy.radius + 5, 0); // Линия в сторону игрока
                    ctx.stroke();
                    ...
                    if (enemy.type === 'melee' && enemy.health > 0 && !useTankTexture) {
                        ctx.fillStyle = 'white';
        ...
        function updateBullets(dt = 1) {
            for (let i = bullets.length - 1; i >= 0; i--) {
                ...
                // --- РИСУЕМ ПУЛИ С ТЕКСТУРАМИ ---
                ctx.save();
                ctx.translate(bullet.x, bullet.y);
                ctx.rotate(bullet.angle);
                
                if (bullet.isRocket && rocketImg.complete && rocketImg.naturalWidth > 0) {
                    // Рисуем ракету RPG с правильной ориентацией
                    // Поворачиваем на 90 градусов (Math.PI/2) для правильной ориентации
                    ctx.rotate(Math.PI/2);
                    ctx.drawImage(rocketImg, 
                                  -ROCKET_WIDTH / 2, -ROCKET_HEIGHT / 2, 
                                  ROCKET_WIDTH, ROCKET_HEIGHT);
                } else if (bulletPistolImg.complete && bulletPistolImg.naturalWidth > 0) {
                    // Рисуем текстуру пули для пистолета и автомата
                    // Поворачиваем на 270 градусов (3π/2) для правильной ориентации
                    ctx.rotate(3 * Math.PI / 2);
                    ctx.drawImage(bulletPistolImg, 
                                  -BULLET_PISTOL_SIZE / 2, -BULLET_PISTOL_SIZE / 2, 
                                  BULLET_PISTOL_SIZE, BULLET_PISTOL_SIZE);
                } else {
                    // Запасной круг для обычных пуль
        ...
        function updateEnemyBullets(dt = 1) {
            enemyBullets.forEach(function(bullet, index) {
                ...
                // --- РИСУЕМ ВРАЖЕСКИЕ ПУЛИ С ТЕКСТУРОЙ ---
                ctx.save();
                ctx.translate(bullet.x, bullet.y);
                ctx.rotate(bullet.angle);
                
                if (enemyBulletImg.complete && enemyBulletImg.naturalWidth > 0) {
                    // Рисуем текстуру вражеской пули
                    ctx.drawImage(enemyBulletImg, 
                                  -ENEMY_BULLET_SIZE / 2, -ENEMY_BULLET_SIZE / 2, 
                                  ENEMY_BULLET_SIZE, ENEMY_BULLET_SIZE);
                } else {
                    // Запасной круг если текстура не загружена
            ...
            if (currentWeapon.name === 'Shotgun') {
                for (let i = 0; i < currentWeapon.pellets; i++) {
                    ...
                    bullets.push({
                        ...
                        radius: BULLET_PISTOL_SIZE / 2, // Радиус коллизии под размер текстуры пули
                ...
                bullets.push({
                    ...
                    angle: player.angle,
                    speed: currentWeapon.bulletSpeed,
                    radius: Math.max(ROCKET_WIDTH, ROCKET_HEIGHT) / 2, // Радиус коллизии под размер текстуры
                    damage: currentWeapon.damage,
                    color: currentWeapon.color,
                    isRocket: true
                });
        ...
        // --- ДОБАВЛЯЕМ ТЕКСТУРУ ДЛЯ ОБЫЧНЫХ ВРАГОВ ---
        const enemyImg = new Image();
        enemyImg.src = 'enemy.png'; // Положите enemy.png (103x91) рядом с этим HTML-файлом
        const ENEMY_TEXTURE_WIDTH = 103;
        const ENEMY_TEXTURE_HEIGHT = 91;
        
        // --- ДОБАВЛЯЕМ ТЕКСТУРУ ДЛЯ ИГРОКА ---
        const playerImg = new Image();
        playerImg.src = 'player.png'; // Положите player.png (100x100) рядом с этим HTML-файлом
        const PLAYER_TEXTURE_SIZE = 100;
        
        // --- ДОБАВЛЯЕМ ТЕКСТУРУ ДЛЯ РАКЕТЫ RPG ---
        const rocketImg = new Image();
        rocketImg.src = 'rocket.png'; // Положите rocket.png (17x59) рядом с этим HTML-файлом
        const ROCKET_WIDTH = 17;
        const ROCKET_HEIGHT = 59;
        
        // --- ДОБАВЛЯЕМ ТЕКСТУРУ ДЛЯ ИГРОКА С RPG ---
        const playerRpgImg = new Image();
        playerRpgImg.src = 'player_rpg.png'; // Положите player_rpg.png (100x126) рядом с этим HTML-файлом
        const PLAYER_RPG_WIDTH = 100;
        const PLAYER_RPG_HEIGHT = 126;
        
        // --- ДОБАВЛЯЕМ ТЕКСТУРУ ДЛЯ ПУЛЬ ПИСТОЛЕТА И АВТОМАТА ---
        const bulletPistolImg = new Image();
        bulletPistolImg.src = 'bullet_pistol.png'; // Положите bullet_pistol.png (16x16) рядом с этим HTML-файлом
        const BULLET_PISTOL_SIZE = 16;
        
        // --- ДОБАВЛЯЕМ ТЕКСТУРУ ДЛЯ ВРАЖЕСКИХ ПУЛЬ ---
        const enemyBulletImg = new Image();
        enemyBulletImg.src = 'enemy_bullet.png'; // Положите enemy_bullet.png (32x32) рядом с этим HTML-файлом
        const ENEMY_BULLET_SIZE = 32;
        
        // --- ДОБАВЛЯЕМ ТЕКСТУРУ ДЛЯ ИГРОКА С АВТОМАТОМ ---
        const playerRifleImg = new Image();
        playerRifleImg.src = 'player_rifle.png'; // Положите player_rifle.png (100x126) рядом с этим HTML-файлом
        const PLAYER_RIFLE_WIDTH = 100;
        const PLAYER_RIFLE_HEIGHT = 126;
        
        // --- ДОБАВЛЯЕМ ТЕКСТУРУ ДЛЯ ИГРОКА С ДРОБОВИКОМ ---
        const playerShotgunImg = new Image();
        playerShotgunImg.src = 'player_shotgun.png'; // Положите player_shotgun.png (100x126) рядом с этим HTML-файлом
        const PLAYER_SHOTGUN_WIDTH = 100;
        const PLAYER_SHOTGUN_HEIGHT = 126;
        
        // --- ДОБАВЛЯЕМ ТЕКСТУРУ ДЛЯ ВРАГОВ-ОХОТНИКОВ ---
        const hunterImg = new Image();
        hunterImg.src = 'hunter.png'; // Положите hunter.png (102x106) рядом с этим HTML-файлом
        const HUNTER_TEXTURE_WIDTH = 102;
        const HUNTER_TEXTURE_HEIGHT = 106;
        
        // --- ДОБАВЛЯЕМ ТЕКСТУРУ ДЛЯ ВРАГОВ-ТАНКОВ ---
        const tankImg = new Image();
        tankImg.src = 'tank.png'; // Положите tank.png (109x208) рядом с этим HTML-файлом
        const TANK_TEXTURE_WIDTH = 109;
        const TANK_TEXTURE_HEIGHT = 208;
    </script>
</body>
</html>
